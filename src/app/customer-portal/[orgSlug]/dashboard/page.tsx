...
export default function CustomerDashboardPage({
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const txRes = await fetch(`/api/v1/customers/${customerData.data._id}/transactions?limit=5`, {
        credentials: 'include',
      });
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-semibold">{t('portal.dashboard.currentBalance')}</h2>
            <span className="text-2xl">💰</span>
          </div>
          <p className="text-3xl font-bold">
            {customer.balanceCurrency} {customer.currentBalance.toFixed(2)}
          </p>
          <p className="text-purple-100 text-sm mt-2">{t('portal.dashboard.availableToSpend')}</p>
        </div>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-semibold text-gray-900">{t('portal.dashboard.sendMinutes')}</h2>
            <span className="text-2xl">📱</span>
          </div>
          <p className="text-gray-600">{t('portal.dashboard.sendMinutesDesc')}</p>
        </Link>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-semibold text-gray-900">{t('portal.dashboard.viewHistory')}</h2>
            <span className="text-2xl">📊</span>
          </div>
          <p className="text-gray-600">{t('portal.dashboard.viewHistoryDesc')}</p>
        </Link>
      </div>

      {/* Recent Transactions */}
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-xl font-bold text-gray-900 mb-4">{t('portal.dashboard.recentTransactions')}</h2>
        {transactions.length === 0 ? (
          <p className="text-gray-500 text-center py-8">{t('portal.dashboard.noTransactions')}</p>
        ) : (
